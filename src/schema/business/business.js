...
const BusinessQuery = {
  ...
  businessIsLiked: BusinessTC.addFields({
    likeCount: {
      type: "Int",
      resolve: (business) =>
        business?.favoriteList ? business.favoriteList.length : 0,
    },
    isLiked: {
      type: "Boolean",
      resolve: (business, _, { user }) =>
        business?.favoriteList.indexOf(user?._id) >= 0,
    },
    favoriteList: {
      type: "[MongoID]",
      resolve: () => [],
...
const BusinessMutation = {
  ...
  businessAddPost: BusinessTC.getResolver("businessAddPost", [
    middleware.isAuth,
    middleware.isOwner,
  ]),
  businessRemovePost: BusinessTC.getResolver("businessRemovePost", [
    middleware.isAuth,
    middleware.isOwner,
  ]),
  businessAddEvent: BusinessTC.getResolver("businessAddEvent", [
    middleware.isAuth,
    middleware.isOwner,
  ]),
  businessRemoveEvent: BusinessTC.getResolver("businessRemoveEvent", [
    middleware.isAuth,
    middleware.isOwner,
  ]),
